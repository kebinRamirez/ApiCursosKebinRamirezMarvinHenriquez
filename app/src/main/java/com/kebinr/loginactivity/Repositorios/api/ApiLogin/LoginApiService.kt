package com.kebinr.loginactivity.Repositorios.api.ApiLogin
...
class LoginApiService {
    ...
    fun signIn(user: User) : MutableLiveData<User> {
        ...
        getRestEngine()
            .signIn(user).enqueue(object: Callback<User> {
    ...
    fun signUp(user: User) : MutableLiveData<User> {
        ...
        getRestEngine()
            .signUp(user).enqueue(object: Callback<User> {
            override fun onResponse(call: Call<User>, response: Response<User>) {
                if (response.isSuccessful) {
                    Log.d("MyOut", "OK isSuccessful " + response.body())
                    val loginResponse = response.body()
    ...
    suspend fun signUp2(user: User) {
        getRestEngine()
            .signUp2(user.email, user.password, user.username, user.name).enqueue(object:
